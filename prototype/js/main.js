...
function onBtnRecordClicked (){
	if (localStream == null) {
		...
		mediaRecorder.onstart = function(){
			...
			log('🚀 ~ file: main.js:126 ~ onBtnRecordClicked ~ log:', log);
		...
		mediaRecorder.onstop = function(){
			...
			*/
			...
			console.log('🚀 ~ file: main.js:186 ~ onBtnRecordClicked ~ downloadLink:', downloadLink);
			sendToAirtable(name);
		};
...
async function sendToAirtable(fileName) {
	console.log('🚀 ~ SENDING TO AIRTABLE:: ', fileName);
	const cleanUrl = downloadLink.href.replace('blob:', '');
	try {
			const data = {
				fileName: fileName,
			...
			console.log('🚀 ~ file: main.js:472 ~ sendToAirtable ~ response:', response);
			...
			dataElement.innerHTML += '<br>Successfully uploaded to Airtable!';

	} catch (error) {
			console.error('Error uploading to Airtable:', error);
			// Add visual feedback for error
			const dataElement = document.querySelector('#data');
			dataElement.innerHTML += '<br>Error uploading to Airtable: ' + error.message;
	}
}
